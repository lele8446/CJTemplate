...
// 1. 默认自动在 +load 方法中向CJModuleManager进行注册
// 2. 必须实现 CJLModuleProtocol 协议方法：+sharedInstance，-setup
// 3. 在程序启动时会自动触发 -setup 方法，请在该方法内进行组件相关初始化及配置。
// 4. 根据实际业务需求，在适当的地方通过 CJModuleManager 触发消息广播。
// 5. 根据实际业务需求，有选择的实现 CJModuleProtocol 协议方法。
...
@interface CJTemplateModule : NSObject<CJLModuleProtocol>
